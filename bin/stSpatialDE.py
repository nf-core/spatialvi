...
parser.add_argument('--saveFileName',
                    metavar='file',
                    type=str,
                    default="stSpatialDE.csv",
                    help='File name.')
parser.add_argument('--savePlotName',
                    metavar='plot',
                    type=str,
                    default="stSpatialDE.png",
                    help='File name.')
parser.add_argument('--plotTopHVG',
                    metavar='number',
                    type=int,
                    default=15,
                    help='File name.')
parser.add_argument('--numberOfColumns',
                    metavar='number',
                    type=int,
                    default=5,
                    help='File name.')
...
counts = pd.DataFrame(st_adata.X.todense(),
                      columns=st_adata.var_names,
                      index=st_adata.obs_names)
coord = pd.DataFrame(st_adata.obsm['spatial'],
...
sc.pl.spatial(st_adata,
              ...
              color=keys,
              alpha=0.7,
              save=args.savePlotName,
              ncols=args.numberOfColumns)
