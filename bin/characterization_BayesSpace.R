...
args <- parser$add_argument_group("Arguments", "required and optional arguments")
...
clusterPlot(dsp, palette = c("purple", "cyan", "blue", "yellow", "red"), color = NA) +
    theme_bw() +
    xlab("Column") +
    ylab("Row") +
...
clusterPlot(dsp.enhanced, palette = c("purple", "cyan", "blue", "yellow", "red"), color = NA) +
    theme_bw() +
    xlab("Column") +
    ylab("Row") +
    labs(fill = "BayesSpace\ncluster", title = "Spatial clustering")
...
write.csv(df_subspot_cluster_and_coord, file = paste0(args$bayesSubspotCoord))

df_enhanced_markers <- as.data.frame(dsp.enhanced@assays@data@listData[["logcounts"]][markers,])
write.csv(df_enhanced_markers, file = paste0(args$bayesEnhancedMarkers))

df_spot_cluster <- subset(as.data.frame(dsp@colData), select = -c(imagecol, imagerow))
write.csv(df_spot_cluster, file = paste0(args$bayesSpotCluster))
