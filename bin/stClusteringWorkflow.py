...
parser.add_argument('--fileNameST',
                    metavar='name',
                    type=str,
                    default='st_adata_norm.h5ad',
                    help='')
parser.add_argument('--fileNameSC',
                    metavar='name',
                    type=str,
                    default='sc_adata_norm.h5ad',
                    help='')
parser.add_argument('--STdeconvolveSCclusterIds',
                    metavar='name',
                    type=str,
                    default='STdeconvolve_sc_cluster_ids.csv',
                    help='')
parser.add_argument('--STdeconvolvePropNormName',
                    metavar='name',
                    type=str,
                    default='STdeconvolve_prop_norm.csv',
                    help='')
parser.add_argument('--STdeconvolveBetaNormName',
                    metavar='name',
                    type=str,
                    default='STdeconvolve_beta_norm.csv',
                    help='')
parser.add_argument('--STdeconvolveSCloadings',
                    metavar='name',
                    type=str,
                    default='STdeconvolve_sc_pca_feature_loadings.csv',
                    help='')
parser.add_argument('--STdeconvolveSCclusterMarkers',
                    metavar='name',
                    type=str,
                    default='STdeconvolve_sc_cluster_markers.csv',
                    help='')
parser.add_argument('--SPOTlightPropNorm',
                    metavar='name',
                    type=str,
...
parser.add_argument('--resolution',
                    ...
                    default=0.4,
                    help='')
parser.add_argument('--scanpy_UMAP_st_sc',
                    metavar='name',
                    type=str,
                    default='scanpy_UMAP_st_sc.png',
                    help='')
parser.add_argument('--scanpy_UMAP_st_sc_BBKNN',
                    metavar='name',
                    type=str,
                    default='scanpy_UMAP_st_sc_BBKNN.png',
                    help='')
parser.add_argument('--scanorama_UMAP_st_sc',
                    metavar='name',
                    type=str,
                    default='scanorama_UMAP_st_sc.png',
                    help='')
parser.add_argument('--LT_individual_histograms',
                    metavar='name',
                    type=str,
                    default='LT_individual_histograms.png',
                    help='')
parser.add_argument('--LT_individual_histograms_combined',
                    metavar='name',
                    type=str,
                    default='LT_individual_histograms_combined.png',
                    help='')
parser.add_argument('--Topics_LDA_spatial',
                    metavar='name',
                    type=str,
                    default='Topics_LDA_spatial.png',
                    help='')
parser.add_argument('--Topics_NMF_spatial',
                    metavar='name',
                    type=str,
                    default='Topics_NMF_spatial.png',
                    help='')
parser.add_argument('--Topics_LT_PC_spatial',
                    metavar='name',
                    type=str,
                    default='Topics_LT_PC_spatial.png',
                    help='')
parser.add_argument('--UMAP_st_spots_clusters',
                    metavar='name',
                    type=str,
                    default='UMAP_st_spots_clusters.png',
                    help='')
parser.add_argument('--UMAP_clusters_embedding_density',
                    metavar='name',
                    type=str,
                    default='UMAP_clusters_embedding_density.png',
                    help='')
parser.add_argument('--UMAP_LDA_topics',
                    metavar='name',
                    type=str,
                    default='UMAP_LDA_topics.png',
                    help='')
parser.add_argument('--UMAP_NMF_topics',
                    metavar='name',
                    type=str,
                    default='UMAP_NMF_topics.png',
                    help='')
parser.add_argument('--UMAP_LT_PC_topics',
                    metavar='name',
                    type=str,
                    default='UMAP_LT_PC_topics.png',
                    help='')
parser.add_argument('--Clusters_scanpy_spatial',
                    metavar='name',
                    type=str,
                    default='Clusters_scanpy_spatial.png',
                    help='')
parser.add_argument('--violin_topics_LDA',
                    metavar='name',
                    type=str,
                    default='violin_topics_LDA.png',
                    help='')
parser.add_argument('--violin_topics_NMF',
                    metavar='name',
                    type=str,
                    default='violin_topics_NMF.png',
                    help='')
parser.add_argument('--violin_topics_LT_PC',
                    metavar='name',
                    type=str,
                    default='violin_topics_LT_PC.png',
                    help='')
parser.add_argument('--saveFileST',
                    metavar='savefile',
                    type=str,
                    default='st_adata_processed.h5ad',
                    help='Path to a file to save h5ad data into.')
parser.add_argument('--saveFileSC',
                    metavar='savefile',
                    type=str,
...
df_st = pd.DataFrame(index=st_adata.var.index, data=st_adata.X.T.todense(),
                     columns=st_adata.obs.index)
df_sc = pd.DataFrame(index=sc_adata.var.index, data=sc_adata.X.T.todense(),
                     columns=sc_adata.obs.index)
...
se_clusters_SC = pd.read_csv(args.STdeconvolveSCclusterIds, index_col=0).rename({'x': 'cluster_id'}, axis=1)['cluster_id']
...
sc.pl.umap(st_adata, color=["clusters", "total_counts", "n_genes_by_counts"],
           wspace=0.4, save=args.UMAP_st_spots_clusters)
sc.tl.embedding_density(st_adata, basis='umap', groupby='clusters')
sc.pl.embedding_density(st_adata, groupby='clusters', ncols=10,
                        save=args.UMAP_clusters_embedding_density)

# Plot LDA cell topics proportions in UMAP
if True:
    plt.rcParams["figure.figsize"] = (4, 4)
    keys = st_adata.obs.columns[st_adata.obs.columns.str.contains('Topic LDA ')]
    sc.pl.umap(st_adata, color=keys, wspace=0.4, ncols=5,
               save=args.UMAP_LDA_topics)
    keys = st_adata.obs.columns[st_adata.obs.columns.str.contains('Topic NMF ')]
    sc.pl.umap(st_adata, color=keys, wspace=0.4, ncols=5,
               save=args.UMAP_NMF_topics)
    keys = st_adata.obs.columns[st_adata.obs.columns.str.contains('LT PC ')]
    sc.pl.umap(st_adata, color=keys, wspace=0.4, ncols=5,
               save=args.UMAP_LT_PC_topics)

# Make plots of spatial ST spots clusters
if True:
    plt.rcParams["figure.figsize"] = (10, 10)
    sc.pl.spatial(st_adata, img_key="hires", color=["clusters"],
                  save=args.Clusters_scanpy_spatial)  # groups=['1', '2', '3']

# Make violin plots of topic proportions of ST spots by clusters
if True:
    plt.rcParams["figure.figsize"] = (3.5, 3.5)
    keys = st_adata.obs.columns[st_adata.obs.columns.str.contains('Topic LDA ')]
    sc.pl.violin(st_adata, keys, jitter=0.4, groupby='clusters', rotation=0,
                 save=args.violin_topics_LDA)
    keys = st_adata.obs.columns[st_adata.obs.columns.str.contains('Topic NMF ')]
    sc.pl.violin(st_adata, keys, jitter=0.4, groupby='clusters', rotation=0,
                 save=args.violin_topics_NMF)
    keys = st_adata.obs.columns[st_adata.obs.columns.str.contains('LT PC ')]
    sc.pl.violin(st_adata, keys, jitter=0.4, groupby='clusters', rotation=0,
                 save=args.violin_topics_LT_PC)
