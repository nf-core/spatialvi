...
for table_name, adata in sdata.tables.items():
    sc.pp.highly_variable_genes(adata, hvg_flavor="seurat", n_top_genes=n_hvgs, inplace=True)
```
...
for i, table_name in enumerate(sdata.tables.keys()):
    ...
    sdata.tables[table_name].obsm["X_umap"] = adatas_corrected[i].obsm["X_umap"]
    sdata.tables[table_name].obs["clusters"] = adatas_corrected[i].obs["clusters"]
