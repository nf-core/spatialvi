...
args$add_argument("--countsFactor",
                  ...
                  metavar  = "factor",
                  required = FALSE)
args$add_argument("--corpusRemoveAbove",
                  default  = 1.0,
                  type     = "double",
                  help     = "factor",
                  metavar  = "factor",
                  required = FALSE)
args$add_argument("--corpusRemoveBelow",
                  default  = 0.05,
                  type     = "double",
                  help     = "factor",
                  metavar  = "factor",
                  required = FALSE)
args$add_argument("--LDAminTopics",
                  default  = 8,
                  type     = "integer",
                  help     = "factor",
                  metavar  = "factor",
                  required = FALSE)
args$add_argument("--LDAmaxTopics",
                  default  = 9,
                  type     = "integer",
                  help     = "factor",
                  metavar  = "factor",
                  required = FALSE)
args$add_argument("--LDAsaveFile",
                  default  = "STdeconvolve_optLDA.rds",
                  help     = "File to save LDA RDS",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveScatterpiesName",
                  default  = "STdeconvolve_st_scatterpies.png",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveScatterpiesSize",
                  default  = 2.85,
                  type     = "double",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveFeaturesSizeFactor",
                  default  = 1.0,
                  type     = "double",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveFeaturesName",
                  default  = "STdeconvolve_st_prop.png",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveCorrName",
                  default  = "STdeconvolve_st_prop_corr.png",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolvePropNormName",
                  default  = "STdeconvolve_prop_norm.csv",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveBetaNormName",
                  default  = "STdeconvolve_beta_norm.csv",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveSCclustersName",
                  default  = "STdeconvolve_sc_clusters.png",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveSCclusterIds",
                  default  = "STdeconvolve_sc_cluster_ids.csv",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveSCpca",
                  default  = "STdeconvolve_sc_pca.csv",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveSCloadings",
                  default  = "STdeconvolve_sc_pca_feature_loadings.csv",
                  help     = "dir",
                  metavar  = "file",
                  required = FALSE)
args$add_argument("--STdeconvolveSCclusterMarkers",
...
if (!is.na(filename)) {
    print(filename)
    se_st <- Seurat::Load10X_Spatial(data.dir = args$outsPath,
                                     filename = filename)
} else {
    image <- Read10X_Image(image.dir     = file.path(args$outsPath, 'spatial'),
                           filter.matrix = TRUE)
    m <- Read10X(paste0(args$outsPath, args$outsSubDir),
                 gene.column = args$mtxGeneColumn)
...
ldas <- fitLDA(t(as.matrix(corpus)),
               Ks = seq(args$LDAminTopics, args$LDAmaxTopics, by = 1))
...
ggcorrplot::ggcorrplot(corr         = decon_cor,
                       p.mat        = p.mat[[1]],
                       hc.order     = TRUE,
                       type         = "full",
                       insig        = "blank",
...
se_sc <- Seurat::CreateSeuratObject(counts = as((args$countsFactor) * matrix_sc,
                                                "sparseMatrix"))
...
write.csv(se_sc@active.ident,
          file = args$STdeconvolveSCclusterIds)
write.csv(se_sc@reductions[["pca"]]@cell.embeddings,
          file = args$STdeconvolveSCpca)
write.csv(se_sc@reductions[["pca"]]@feature.loadings,
          file = args$STdeconvolveSCloadings)
write.csv(cluster_markers_all,
          file = args$STdeconvolveSCclusterMarkers)
