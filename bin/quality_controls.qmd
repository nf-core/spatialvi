...
```{python}
del sdata.tables[f"{sample_id}_table"]
sdata.tables[f"{sample_id}_table"] = adata
# Filtering spatialdata elements based on the anndata elements:
region, _, instance_key  = get_table_keys(sdata["Visium_HD_Human_Lung_Cancer_HD_Only_D_table"])
# If region is a list, take the first element
if isinstance(region, list):
    region = region[0]
matched_element, _ = spatialdata.match_element_to_table(sdata, region, f"{sample_id}_table")
sdata[region] = matched_element[region]
