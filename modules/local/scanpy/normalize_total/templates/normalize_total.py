...
def normalize_adata(adata, target_sum):
    ...
    if "total_counts" in adata.obs:
        ...
        print(f"Median total counts before normalization: {median_before:.2f}")
...
def main():
    ...
    print(f"Normalizing AnnData from: {h5ad}")
    adata = ad.read_h5ad(h5ad)

    adata = normalize_adata(adata, target_sum=target_sum)

    adata.write_h5ad(output_h5ad)
    print(f"Written normalized AnnData to: {output_h5ad}")
