...
def extract_to_legacy_anndata(sdata, table_name, coord_system):
    ...
    logger.info(f"Using coordinate system: {coord_system}")
...
def main():
    ...
    logger.info(f"Processing SpatialData from: {input_sdata}")
    logger.info(f"Sample ID: {sample_id}")
    sdata = spatialdata.read_zarr(input_sdata)
    logger.info(f"Tables found: {list(sdata.tables.keys())}")
    logger.info(f"Coordinate systems: {list(sdata.coordinate_systems)}")
